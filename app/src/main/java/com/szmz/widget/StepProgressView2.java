...
public class StepProgressView2 extends View {
    ...
    private static final int DEFAULT_CIRCLE_RADIUS = 3;// dp 默认圆球半径
    private static final int DEFAULT_LINE_LENGTH = 80;// dp 默认的进度线长度
    private static final int DEFAULT_TEXT_SIZE = 10;// sp 默认的字体大小
    private static final int DEFAULT_OFFSET = 3;// dp 默认背景与视图的偏差
    private static final int DEFAULT_STEP_COUNTS = 3;// 默认的步骤数
    private static final int DEFAULT_PADDING = 15;// dp 默认的padding值
    private static final int DEFAULT_REACHED_COLOR = 0x55d83538;// 已经通过的步骤的颜色
    private static final int DEFAULT_CUR_COLOR = 0xffd83538;// 当前的步骤
    ...
    private boolean curStaus = false;

    public boolean isCurStaus() {
        return curStaus;
    }

    public void setCurStaus(boolean curStaus) {
        this.curStaus = curStaus;
    }
...
//			Typeface.NORMAL);
    ...
    public void setStepDesc(List<HdxtGrcxInfo.TotalNodeBean> list) {
        if (list.size() < stepCounts) {
            try {
                throw new Exception("错误，步骤描述过少");
            } catch (Exception e) {
                e.printStackTrace();
            ...
            for (int i = 0; i < stepCounts; i++) {
                stepDesc[i] = list.get(i).getNode();
            }
        }
    }

    public StepProgressView2(Context context, AttributeSet attrs,
                             int defStyleAttr) {
    ...
    public StepProgressView2(Context context) {
        this(context, null);
    }

    private void init(Context context, AttributeSet attrs) {
        // 固定值
        textMargin = dp2px(15, context);
        ...
        textSize = (int) sp2px(ta.getDimension(
                R.styleable.StepProgressView_text_sizes, DEFAULT_TEXT_SIZE),
                context);
